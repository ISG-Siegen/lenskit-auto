...
class UserUser(UserKNNScorer):
    ...
    def get_default_configspace(hyperopt = False, **kwargs):
        """
            return default (hyperopt) configuration spaces for hyperparameter
        """

        if hyperopt:
            cs = {
                "algo": "UserUser",
                "UserUser:max_nbrs": hp.uniformint("UserUser:max_nbrs", 1, 10000),
                "UserUser:min_nbrs": hp.uniformint("UserUser:min_nbrs", 1, 1000),
                "UserUser:min_sim": hp.uniform("UserUser:min_sim", 1.0e-10, 1.0e-2),
            }
